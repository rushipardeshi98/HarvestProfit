...
const Receipt = () => {
  ...
  const { goodsId } = useParams(); //this is productId
  ...
  const endBid = async () => {
    await axios.get(`${URL}/endbid/${farmgood.goodsId}`,
  ...
  const generateemail = async(userEmailId)=>{
    ...
    let message=
    "Goods Name: "+bidentity.goodsName+"\n" 
    +"Farmer Name: "+bidentity.farmerName+"\n"
    +"Farmer Contact: "+bidentity.farmerContact+"\n"
    +"Buyer Name: "+bidentity.buyerName+"\n"
    +"Buyer Contact: "+bidentity.buyerContact+"\n" 
    +"Final Bid Price: "+bidentity.finalprice+"\n"
    +"Quantity: "+bidentity.quantity+"\n"
    +"Total Amount: "+bidentity.totalAmount+"\n";
    ...
    await axios.post(`${URL}/endbidsendemail`,body,
    {headers:{Authorization:`Bearer ${sessionStorage.getItem("userToken")}`}
