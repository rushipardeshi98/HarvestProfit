...
public class FarmerController {
	...
	public String AddGoods(@Valid @RequestBody FarmgoodsDto farmgoodsdto, HttpSession session) {
		...
		System.out.println(farmgoodsdto.getUserEmailId()+"aaa");
		try {
		...
		/*
		 * try { if(signedfarmer==null) {signedfarmer=getmyself(session);}
		 * 
		 * farmgoods.setFarmer(signedfarmer); farmerservice.addFarmGoods(farmgoods);
		 * 
		 * return "Farm good added successfully"; }catch(Exception e) { return
		 * "Error occured while adding Farm good"; }
		 */
	}
	...
	public ResponseEntity<List<FarmgoodsClass>>  ViewGoods(@PathVariable String userEmailId, HttpSession session ) {
		...
		//if(signedfarmer==null) {signedfarmer=getmyself(session);}
	...
	public ResponseEntity<FarmgoodsClass> getSingleGood(@PathVariable String goodsId){
		...
		System.out.println(singlegood.toString());
...
//	public ResponseEntity<UserdetailsClass> getBuyerinfo(@RequestParam String buyerId){
//		UserdetailsClass buyer = userservice.getBuyer(Integer.parseInt(buyerId));
//		return new ResponseEntity<UserdetailsClass>(buyer,HttpStatus.OK);
//	}
